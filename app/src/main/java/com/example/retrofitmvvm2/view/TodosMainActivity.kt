...
class TodosMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        responseViewModel.gettodosdata(object : ApiCallBback {
            override fun onResponse(response: List<DataX>) {
                recyclerView.adapter = RecyclerAdapter(response)
            }

            override fun onFailure(t: Throwable) {
                Toast.makeText(this@TodosMainActivity,t.toString(), Toast.LENGTH_SHORT).show()
            }
