...
class ProductCategoriesMainActivity : AppCompatActivity() {
    private lateinit var recyclerView: RecyclerView
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_product_categories_main)

        recyclerView = findViewById(R.id.recyclerview)
        recyclerView.layoutManager = LinearLayoutManager(this)

        val responseViewModel =  ViewModelProvider(this).get(ModelResponseClass::class.java)
        responseViewModel.getproductcategoriesdata(object : ApiCallBback {
            override fun onResponse(response: List<DataX>) {
                recyclerView.adapter = RecyclerAdapter(response)
            }

            override fun onFailure(t: Throwable) {
                Toast.makeText(this@ProductCategoriesMainActivity,t.toString(), Toast.LENGTH_SHORT).show()
            }
