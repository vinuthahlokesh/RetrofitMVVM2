...
class CategoriesMainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        responseViewModel.getcategoriesdata(object : ApiCallBback {
            override fun onResponse(response: List<DataX>) {
                recyclerView.adapter = RecyclerAdapter(response)
            }

            override fun onFailure(t: Throwable) {
                Toast.makeText(this@CategoriesMainActivity,t.toString(), Toast.LENGTH_SHORT).show()
            }
